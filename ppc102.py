...
class Ppc102Controller(HardwareMotionBase):
    ...
    ***Device not setting Keys/Intr bits correctly so some items are omitted
    ...
    CHAN_ENABLED = 1
    CHAN_DISABLED = 2
    V_MIN = -25.0              # PPC102 amplifier floor (hardware spec)
    V_MAX_DEFAULT = 150.0      # assumed full scale; real value comes from
                               # TPZ_IOSETTINGS (75 / 100 / 150 V units exist)
    COUNT_FULL_SCALE = 32767   # +32767 counts == +100% of V_max
    COUNT_MIN = -32768          # protocol floor (never generated by our maths)
    _v_max = V_MAX_DEFAULT       # actual max voltage, set by TPZ_IOSETTINGS
    # Define bit meanings
    bit_flags = {
    ...
    def set_output_volts(self, channel: int = 1, volts:int = 0):
        """
            Sets voltage going to specific channel
                ...
                -each channel must be enabled
            channel: (int) 1 or 2
            volts: (int) -25V -> 150V [DEFAULT PPC102 Voltage range as example]
            Returns: True or False on successful com send
        ...
        try:
            ...
            # Check voltage range
            if not self.V_MIN <= volts <= self._v_max:
                self.report_error(
                    f"Voltage out of Range: {volts} V "
                    f"(limits {self.V_MIN} to {self._v_max} V)")
                return False

            #VOLTS TO COUNTS CONVERSTION
            converted_volts = int(round(volts / self._v_max * self.COUNT_FULL_SCALE))
            volts_bytes = converted_volts.to_bytes(2, byteorder='little', signed=True)
    ...
    def get_output_volts(self, channel: int = 1) -> Union[int, None]:
        """
            Gathers the current state of a channels voltage
                -must be in open loop
            channel: (int) 1 or 2
            Returns: Voltage state in int (-V_MIN -> +V_MAX)
        ...
        try:
            ...
            if voltage_raw >= 0x8000:
                voltage_raw -= 0x10000
            return round((voltage_raw/ self.COUNT_FULL_SCALE * self._v_max), 3)
        except Exception as e:
            self.report_error(f"Error: {e}")
            self._check_for_reboot()
            return None
    ...
    def set_max_output_voltage(self, channel: int = 1, limit:int = 150) -> bool:
        ...
        try:
            ...
            self._send_command(command)
            time.sleep(self.DELAY)
            self._v_max = limit / 10  # Store max voltage in volts
            return True
        except Exception as e:
            self.report_error(f"Error: {e}")
            self._check_for_reboot()
    ...
    def _set_ppc_pidconsts(self, channel: int = 1, p_const: float = 900.0,
        ...
        raise NotImplementedError("MGMSG_PZ_SET_PPC_PIDCONSTS: Implemented but "
                                                                    "not tested")
        #check Connection
        if not self.is_connected(): #pylint: disable=W0101
            self.report_error("Device is not connected")
            return False
