...
class Testppc102Coms(unittest.TestCase):
    ...
    def test_send_command(self):
        ...
        with patch.object(self.controller, "_send_command") as mock_write:
    ...
    def test_get_loop(self):
        ...
        with patch.object(self.controller, "_send_command") as mock_get_loop:
            self.controller._read_reply = MagicMock(    # pylint: disable=protected-access
                return_value=bytes([0x41, 0x06, 0x01, 0x00, 0x21, 0x01, 0x02]))
    ...
    def test_set_position(self):
        ...
        with patch.object(self.controller, "_send_command") as mock_setpos:
            self.controller.get_loop = MagicMock(return_value=2)
            self.controller.get_enable = MagicMock(return_value=1)
            self.controller.set_pos(channel = 1, pos = 5.0)
            dest = (0x20 + 1) | 0x80
